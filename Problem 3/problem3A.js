...
let trueTotal = 0;
let testRegEx = /[0-9]+/g;
while ((array = testRegEx.exec(schematic)) !== null) {
    fullArray.push(Number(array[0]));
...
const partRegEx = /[^0-9.\s]/g;
const digitRegEx = /[0-9]/g;

function findNumber(index) {
    let numberString = schematic.charAt(index);
    
    let iRight = index + 1;
    while (schematic.charAt(iRight).match(digitRegEx)) {
        numberString += schematic.charAt(iRight);
        iRight++;
    }

    let iLeft = index - 1;
    let character = schematic[iLeft]
    while (character.match(digitRegEx)) {
        numberString = schematic.charAt(iLeft) + numberString;
        iLeft--;
        character = schematic[iLeft];
    ...
    let numIndex = fullArray.indexOf(Number(numberString));
    fullArray.splice(numIndex, 1);
...
let total = 0;
while ((testArray = partRegEx.exec(schematic)) !== null) { 
    let index = testArray.index;

    if (index - 141 >= 0) {
        let topIndex = index - 141;
        let topLeftIndex = topIndex - 1;
        let topRightIndex = topIndex + 1;

        let positions = [schematic[topLeftIndex].match(digitRegEx),schematic[topIndex].match(digitRegEx),schematic[topRightIndex].match(digitRegEx)];
        if (positions[1] != null && (positions[0] != null || positions[2] != null)) {
            positions[0] = null, positions[2] = null;
        }

        for (let i = 0; i < positions.length; i++) {
            if (positions[i] != null) {
                total += findNumber(topIndex + (i - 1));
            }
        }
    }

    if (index + 141 >= 0 && index + 141 < schematic.length) {
        let bottomIndex = index + 141;
        let bottomLeftIndex = bottomIndex - 1;
        let bottomRightIndex = bottomIndex + 1;

        let positions = [schematic[bottomLeftIndex].match(digitRegEx),schematic[bottomIndex].match(digitRegEx),schematic[bottomRightIndex].match(digitRegEx)];
        if (positions[1] != null && (positions[0] != null || positions[2] != null)) {
            positions[0] = null, positions[2] = null;
        }
        ...
        for (let i = 0; i < positions.length; i++) {
            if (positions[i] != null) {
                total += findNumber(bottomIndex + (i - 1));
            }
        }
    }

    if (schematic.charAt(index + 1).match(digitRegEx)) {
        total += findNumber(index + 1);
    }
    if (schematic.charAt(index - 1).match(digitRegEx)) {
        total += findNumber(index - 1);
    }
...
console.log(total);
let testTotal = 0;
fullArray.forEach(number => testTotal += number);
