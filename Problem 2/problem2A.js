let rawInput = `Game 1: 14 green, 8 blue, 9 red; 5 blue, 4 green, 2 red; 4 red, 4 blue, 4 green; 1 blue, 3 green, 2 red; 10 red, 3 blue, 15 green; 2 red, 6 green, 3 blue
...
for (let i = 0; i < inputArray.length; i++) {
    let skipped = false;
    let game = inputArray[i];

    let redNum = [...game.matchAll(/red/g)].length;
    let blueNum = [...game.matchAll(/blue/g)].length;
    let greenNum = [...game.matchAll(/green/g)].length;

    let redIndex = game.indexOf("red");
    let redOutString = game;
    while (redIndex != -1) {
        let number = redOutString.substring(redIndex - 3, redIndex).trim();
        redOutString = redOutString.substring(redIndex + 3, game.length);
        redIndex = redOutString.indexOf("red");

        if (Number(number) > limits.get("red")) skipped = true;
    }

    let blueIndex = game.indexOf("blue");
    let blueOutString = game;
    while (blueIndex != -1) {
        let number = blueOutString.substring(blueIndex - 3, blueIndex).trim();
        blueOutString = blueOutString.substring(blueIndex + 3, game.length);
        blueIndex = blueOutString.indexOf("blue");

        if (Number(number) > limits.get("blue")) skipped = true;
    }

    let greenIndex = game.indexOf("green");
    let greenOutString = game;
    while (greenIndex != -1) {
        let number = greenOutString.substring(greenIndex - 3, greenIndex).trim();
        greenOutString = greenOutString.substring(greenIndex + 3, game.length);
        greenIndex = greenOutString.indexOf("green");

        console.log(number);
        if (Number(number) > limits.get("green")) skipped = true;
    }
